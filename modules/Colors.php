...
function ColorLerp($color1,$color2,$percent){
    $c = new Color($color1);
    return $c->mix($color2,$percent);

    $redA = $color1 & 0xFF0000;
    $greenA = $color1 & 0x00FF00;
    $blueA = $color1 & 0x0000FF;
    $redB = $color2 & 0xFF0000;
    $greenB = $color2 & 0x00FF00;
    $blueB = $color2 & 0x0000FF;

    $redC = $redA + (($redB - $redA) * $percent) & 0xFF0000;         // Only Red
    $greenC = $greenA + (($greenB - $greenA) * $percent) & 0x00FF00; // Only Green
    $blueC = $blueA + (($blueB - $blueA) * $percent) & 0x0000FF;     // Only Blue
...
/**
 * https://github.com/mexitek/phpColors/blob/master/src/Mexitek/PHPColors/Color.php
 */
use Exception;
